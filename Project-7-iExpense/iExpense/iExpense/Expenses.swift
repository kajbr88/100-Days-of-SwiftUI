...
/* for iOS 17 and above remove ObservableObject protocol and use @Observable here in this line and remove @Published for properties in observed class & change @StateObject for the updating property in ContentView to @State, and also import Observation. */
class Expenses: ObservableObject {
    @Published var itemsPersonal = [ExpenseItem]() { // challenge 3
        ...
        didSet { /* didSet is a property observer in Swift that allows you to execute
                  custom code whenever a property's value changes. Here, whenever the itemsPersonal property is assigned a new value, the didSet block will be executed, and the didSet block will be executed. */
            if let encoded = try? JSONEncoder().encode(itemsPersonal ) { //encode data in swift Object format to JSON format.
                UserDefaults.standard.set(encoded, forKey: "ItemsPersonal") /* save data to
key named "ItemsPersonal"(ItemsPersonal's type is Object) through UserDefaults mechanism in JSON format. UserDeults is suited for simple key-value data queries but not for large datasets or complex queries. */
            }
        }
    }
