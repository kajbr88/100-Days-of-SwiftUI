    ...
    var body: some View {
        ...
        ZStack {
            Spirograph(innerRadius: Int(self.innerRadius), outerRadius: Int(self.outerRadius), distance: Int(self.distance), amount: self.amount)
                ...
                .offset(CGSize(width: 0, height: -185)) /* Spirograph shape is moved 185 points upwards from its original position to position the shape appropriately within the overall view, ensuring it's visible
 and doesn't overlap with the controls below it.*/
